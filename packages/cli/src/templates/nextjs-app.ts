...
export function nextjsAppTemplate(options: TemplateOptions): string {
...
const {
  ...
  if (hasSyntax) {
    ...
    pluginSetup.push(`
// Create syntax highlight plugin (configure languages as needed)
...
const mediaPlugin = createMediaPreviewPlugin();`);
  ...
  if (hasTurnstile) {
    ...
    pluginSetup.push(`
// Create Turnstile plugin (add your site key)
const turnstilePlugin = createTurnstilePlugin({
  ...
  if (hasI18n) {
    component = `
export default function ThreadKitDemoPage() {
...
` : ''}
      <ThreadKit
        projectId={process.env.NEXT_PUBLIC_THREADKIT_PROJECT_ID || '${projectId}'}
        url="/threadkit-demo"
        ${pluginProps.join('\n        ')}
      />
    </div>
