...
function renderTokens(tokens: Token[], options: MarkdownOptions): string {
...
function renderMarkdownLines(text: string, options: MarkdownOptions): string {
  ...
  const flushQuote = () => {
    if (quoteLines.length > 0) {
      const content = quoteLines.map((line, i) =>
        renderTokens(tokenizeLine(line, options), options) + (i < quoteLines.length - 1 ? '<br>' : '')
      ).join('');
  ...
  const flushParagraph = () => {
    if (paragraphLines.length > 0) {
      const content = paragraphLines.map((pLine, pi) =>
        renderTokens(tokenizeLine(pLine, options), options) + (pi < paragraphLines.length - 1 ? '<br>' : '')
      ).join('');
