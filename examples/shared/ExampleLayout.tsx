...
cargo run --release --bin threadkit-http -- \
  ...
  --enable-auth email,google,github,anonymous
      ...
      <footer style={{ marginTop: 32, color: '#666', fontSize: 14 }}>
        <p>
          <strong>API Endpoint:</strong> <code>http://localhost:8080/v1</code>
        </p>
        <p>
          <strong>WebSocket:</strong> <code>ws://localhost:8081</code>
        </p>
        <p>
          <strong>Project ID:</strong> <code>tk_pub_your_public_key</code>
        </p>
      </footer>
    </div>
  );
